...
std::vector<float> cpu_reference(const VectorCase& test_case) {
    std::vector<float> expected(static_cast<size_t>(test_case.N), 0.0f);
    for (int i = 0; i < test_case.N; ++i) {
        expected[i] = test_case.A[i] + test_case.B[i];
    }
    return expected;
}
...
VectorCase load_case_from_file(const fs::path& path) {
    ...
    test_case.A.resize(static_cast<size_t>(test_case.N));
    test_case.B.resize(static_cast<size_t>(test_case.N));
...
VectorCase make_random_case(int case_index, std::mt19937& rng) {
...
fs::path default_test_dir(const char* argv0) {
    ...
    const fs::path cwd_dir = fs::current_path() / "vector_add" / "testdata";
...
bool run_case(const VectorCase& test_case) {
    const std::vector<float> expected = cpu_reference(test_case);
    std::vector<float> actual(static_cast<size_t>(test_case.N), 0.0f);
    ...
    try {
        CUDA_CHECK(cudaMemcpy(d_a, test_case.A.data(), bytes, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(d_b, test_case.B.data(), bytes, cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemset(d_c, 0, bytes));

        solve(d_a, d_b, d_c, test_case.N);
...
void print_usage(const char* argv0) {
    ...
    std::cout << "  " << argv0 << " --case ./testdata/case_00_sanity_4.txt\n";
}
