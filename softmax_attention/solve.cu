...
// generated by Codex: row-wise softmax built on top of existing getmax/reduction helpers
void softmax_row(const float *input, float *output, int N) {
    const int threadsPerBlock = 256;
    float *pivot = nullptr;
    cudaMalloc(&pivot, static_cast<size_t>(N) * sizeof(float));

    const float row_max = getmax(input, N);
    elementwise_sub_and_exp_kernel<<<(N + threadsPerBlock - 1) / threadsPerBlock, threadsPerBlock>>>(input, output, row_max, N);
    const float row_sum = reduction(output, N);
    elementwise_divide<<<(N + threadsPerBlock - 1) / threadsPerBlock, threadsPerBlock>>>(output, row_sum, N);

    cudaFree(pivot);
}

// generated by Codex: apply the row softmax to each attention score row
void softmax_rows(const float *input, float *output, int M, int N) {
    for (int m = 0; m < M; ++m) {
        softmax_row(input + static_cast<size_t>(m) * N, output + static_cast<size_t>(m) * N, N);
    }
}
